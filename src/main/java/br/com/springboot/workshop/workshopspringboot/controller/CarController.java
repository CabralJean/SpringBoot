package br.com.springboot.workshop.workshopspringboot.controller;
...
import br.com.springboot.workshop.workshopspringboot.service.CarService;
...
public class CarController {
	...
	public ResponseEntity<Car> create(@RequestBody @Validated final Car car) throws Exception {
	...
	public ResponseEntity<Car> getAll(@PathVariable(name = "car_id", required = true) String id) {

		final Car car = this.carService.findOne(id);

		if (car == null) {
			return new ResponseEntity<Car>(this.carService.findOne(id), HttpStatus.NOT_FOUND);
		}

		return new ResponseEntity<Car>(this.carService.findOne(id), HttpStatus.OK);
	}
